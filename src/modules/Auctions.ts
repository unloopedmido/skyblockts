...
export type Tier = 'COMMON' | 'UNCOMMON' | 'RARE' | 'EPIC' | 'LEGENDARY' | 'MYTHIC' | 'DIVINE' | 'SPECIAL' | 'SUPERIOR' | 'ULTIMATE';
...
export class Auctions {
    ...
    private filterAuctions(auctions: AuctionItem[], options: {
        tier?: Tier;
        minPrice?: number;
        maxPrice?: number;
        binOnly?: boolean;
        ...
        return auctions.filter(auction => {
            ...
            if (options.minPrice && auction.starting_bid < options.minPrice) isValid = false;
            if (options.maxPrice && auction.starting_bid > options.maxPrice) isValid = false;
            if (options.binOnly && !auction.bin) isValid = false;
            if (options.itemName && !auction.item_name.toLowerCase().includes(options.itemName.toLowerCase())) isValid = false;
            if (options.auctioneer && auction.auctioneer !== options.auctioneer) isValid = false;
            return isValid;
        });
    }

    private async getAuctionsWithCache(): Promise<AuctionItem[]> {
        if (this.cache.auctions && (Date.now() - this.cache.timestamp) < this.client.cacheTTL) {
    ...
    async search(query: string): Promise<AuctionItem[] | null> {
        const auctions = await this.getAuctionsWithCache();
        return auctions.filter(auction => auction.item_name.toLowerCase().includes(query.toLowerCase())) || [];
    }
    ...
    async getAuctioneer(auctioneerUUID: string): Promise<AuctionItem[] | null> {
        const auctions = await this.getAuctionsWithCache();
        return auctions.filter(auction => auction.auctioneer === auctioneerUUID) || [];
    }
    ...
    async all(): Promise<AuctionItem[]> {
        try {
            ...
            for (let i = 0; i < totalPages - 1; i += batchSize) {
                const batch = Array.from({ length: Math.min(batchSize, totalPages - 1 - i) })
                    .map((_, idx) => {
                        const page = i + idx + 1;
